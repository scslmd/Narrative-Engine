# Story Development Canonical Contract v1.3
...
### 5.2B Pattern Extraction Objects

Pattern Extraction generalizes Mythos Extraction and supports both `narrative` and `mythology` source types. When `source_type` is `"mythology"`, the service delegates to `MythosExtractionService`. When `source_type` is `"narrative"`, it uses the narrative-specific extraction pipeline with additional voice profile, narrative pattern, and thematic constraint fields.

#### Canonical Objects

**NarrativePattern**
- `pacing`: string — overall pacing characteristics (e.g., "deliberate, scene-driven")
- `chapter_structure`: string — how chapters are structured (e.g., "single POV, cliffhanger endings")
- `conflict_type`: string — primary conflict mode (e.g., "internal vs. external tension")
- `dialogue_style`: string — dialogue characteristics (e.g., "terse, subtext-heavy")
- `scene_transition`: string — how scenes transition (e.g., "hard cuts, temporal jumps")

**VoiceProfile**
- `narrative_voice`: string — overall narrative voice (e.g., "close third-person, intimate")
- `sentence_rhythm`: string — sentence-level patterns (e.g., "short declarative sentences with occasional long descriptive passages")
- `descriptive_density`: string — level of description (e.g., "sparse, focused on action")
- `humor_level`: string — humor presence (e.g., "dry wit, ironic observations")
- `emotional_temperature`: string — emotional register (e.g., "restrained, with moments of intensity")

**ThematicConstraint**
- `theme`: string — the thematic concern (e.g., "identity and memory")
- `moral_stance`: string — moral position on the theme (e.g., "ambiguous, no easy answers")
- `recurring_questions[]`: list[string] — questions the story keeps asking
- `forbidden_elements[]`: list[string] — elements the story avoids

**WorldRule**
- `rule`: string — governing rule of the world
- `enforcement`: string — mechanism that enforces the rule
- `exceptions[]`: list[string] — known exceptions or loopholes

**StoryEntity**
- `name`: string — entity name
- `entity_type`: string — type of entity (e.g., "character", "location", "organization")
- `archetype`: string — archetypal role
- `domain_or_power`: string — sphere of influence or expertise
- `canonical_facts[]`: list[string] — immutable facts about the entity

**PatternExtractionAnalysis**
- Container dataclass with: source_type, source_corpus, generation_mode, archetypal_patterns[], narrative_structures[], world_rules[], symbolic_motifs[], thematic_spine, emotional_promise, tone_and_voice_direction, narrative_pattern (narrative-only), voice_profile (narrative-only), thematic_constraints[], key_entities[], entity_relationships[]

#### Source Types

- `narrative` — fiction stories; uses full extraction pipeline with voice profile and narrative pattern
- `mythology` — mythological texts; delegates to MythosExtractionService

#### Generation Modes

- `same_world` — use the source story's established world and characters
- `new_characters` — same world, original cast fulfilling extracted archetypes
- `transposed` — map patterns to a new setting; adapt voice, structure, and themes

#### Canonical Naming

- Source types are lowercase: "narrative", "mythology"
- Generation modes are snake_case: "same_world", "new_characters", "transposed"
- Pattern names use kebab-case: "hubris-fall-redemption", "trickster-disguise"
- Thematic constraint themes use sentence case
...
| **PatternGuidance** | Structured pattern context for P-100/P-300 prompts: voice profile, world rules, thematic constraints from PatternExtractionAnalysis | `app/services/scene_context.py` |
...
Pattern-guided drafting rules:

- SceneContext extended with `pattern_guidance` field for voice profile, world rules, and thematic constraints
- P-100 Architect prompt adapted with `_build_pattern_context_block()` for pattern context injection across 3 generation modes
- P-300 Drafter includes pattern guidance + per-chapter author direction via SceneContext's `author_prompt` field
...
| `extract_patterns_*` | source text, source_type (narrative or mythology), generation_mode (same_world, new_characters, transposed) | pattern extraction analysis with archetypal patterns, narrative structures, voice profile, thematic constraints, world rules, entities | create structured pattern objects for downstream story generation |
| `generate_from_patterns_*` | pattern extraction id plus generation mode (same_world, new_characters, transposed) | generated story artifacts seeded from extracted patterns | create new story content informed by extracted narrative DNA |
