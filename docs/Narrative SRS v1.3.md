...
```json
{
  ...
  "config": {
    ...
    "mythos_source_corpus": "string",
    "mythos_generation_mode": "string",
    "pattern_source_type": "string",
    "pattern_generation_mode": "string",
    "pattern_source_corpus": "string"
  },
  "constraints": [
    "string"
  ]
...
- `POST /projects/import-patterns` - Analyze any story or mythology text and extract storytelling DNA including archetypal patterns, narrative structure, voice profile, thematic constraints, world rules, and entities; supports narrative and mythology source types with three generation modes (201 Created)
- `POST /projects/{project_id}/extract-patterns` - Extract patterns from an existing project's manuscript documents for pattern-guided generation (201 Created)
...
- `P-300` builds a drafter inference request from manifest plus upstream architect, sequence, character anchors, world constraints, prior chapter context (last 3 chapters), and pattern guidance (voice profile, world rules, thematic constraints) and registers canonical `chapter_{chapter_id}` (or `chapter_1` for backward-compatible single-chapter mode)
...
- `P-300` (Multi-Chapter Mode — with `chapter_id` in payload)
  ...
  - pattern guidance injection: voice profile, world rules, thematic constraints from PatternExtractionAnalysis
- `P-400`
...
- `SceneContext` extended with `pattern_guidance` and `author_prompt` fields for pattern-guided generation
...
Current test baseline: **1031 passed, 9 skipped**
...
### 17.5B Pattern Extraction

Pattern Extraction is the generalized pattern extraction service that analyzes any story or mythology text and extracts its "storytelling DNA" — archetypal patterns, narrative structure, voice profile, thematic constraints, world rules, and entities. Extracted patterns guide original story generation through three modes: same-world (use established world/characters), new-characters (same world, original cast), or transposed (map patterns to new setting).

Pattern Extraction generalizes Mythos Extraction: when `source_type` is `"mythology"`, the service delegates to `MythosExtractionService`. When `source_type` is `"narrative"`, it uses the narrative-specific extraction pipeline with `build_narrative_analysis_request()`.
...
- `text`: string, required, min_length=1, max_length=5_000_000
- `source_type`: string, required, validated against ("narrative", "mythology")
- `generation_mode`: string, default "same_world", validated against ("same_world", "new_characters", "transposed")
- `project_id`: string | null, optional
- `source_corpus`: string | null, optional
...
- `status`: "completed" | "failed"
- `project_id`: string
- `extraction`: PatternExtractionSummary | null — { source_corpus, archetypal_patterns: int, narrative_structures: int, world_rules: int, symbolic_motifs: int }
- `error`: string | null

**Endpoint:** `POST /projects/{project_id}/extract-patterns`
**Status Code:** 201 Created
**Request Schema (ExtractPatternsRequest):**
- `source_type`: string, required, validated against ("narrative", "mythology")
- `generation_mode`: string, default "same_world", validated against ("same_world", "new_characters", "transposed")
- `source_corpus`: string | null, optional

### Data Models (app/schemas/pattern_extraction.py)

**NarrativePattern:** pacing, chapter_structure, conflict_type, dialogue_style, scene_transition
**VoiceProfile:** narrative_voice, sentence_rhythm, descriptive_density, humor_level, emotional_temperature
**ThematicConstraint:** theme, moral_stance, recurring_questions[], forbidden_elements[]
**WorldRule:** rule, enforcement, exceptions[]
**StoryEntity:** name, entity_type, archetype, domain_or_power, canonical_facts[]
**PatternExtractionAnalysis:** container with source_type, source_corpus, generation_mode, archetypal_patterns[], narrative_structures[], world_rules[], symbolic_motifs[], thematic_spine, emotional_promise, tone_and_voice_direction, narrative_pattern (narrative-only), voice_profile (narrative-only), thematic_constraints[], key_entities[], entity_relationships[]

### Persistence Contract

- Foundation Profile: thematic_spine, emotional_promise, tone_direction set from analysis; narrative_constraints stores archetypal patterns + narrative structures + thematic constraints as JSON
...
ManifestConfig extended with:
- `pattern_source_type`: string, default "" (narrative | mythology)
- `pattern_generation_mode`: string, default "" (same_world | new_characters | transposed)
- `pattern_source_corpus`: string, default ""

### SceneContext Extension

SceneContext dataclass extended with:
...
- P-300 Drafter includes pattern guidance (voice profile, world rules, thematic constraints) + per-chapter author direction via SceneContext
- Pattern guidance influences tone, structure, and character voice during drafting

### Error Handling

- `PatternExtractionError(ValueError)` — caught, returns status="failed"
- `InferenceBackendError` — caught, returns status="failed" with error code
...
8. arc recommendation and stage mapping
9. planning objects
10. drafting generation and continuation
11. suggestions and revision
12. continuity and review
13. inspect and provenance polish
14. orchestrator hardening and retry coverage
