# Executor Prompt: RH-T048 - Apply final semantics to StudioStatusBar
...
- Responsible file: `frontend/src/components/studio/StudioStatusBar.tsx`
- Serial dependencies: RH-T023
- Same-file serial group: studio-status-bar

## Objective
Add aria-live support to status text and keep the bar non-interactive unless a real action exists.
...
## Task Guardrails
- Do not add role="button" or click handlers unless a real navigation/status action is implemented in the same task.
...
## Instructions
1. Modify only `frontend/src/components/studio/StudioStatusBar.tsx`.
