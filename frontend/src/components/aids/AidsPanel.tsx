...
export function AidsPanel({
  ...
  return (
    <div className="flex h-full w-96 flex-col border-l border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900">
      ...
      <div className="flex border-b border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 dark:bg-slate-800">
        <button
            ...
            {openSuggestions.length === 0 ? (
              ...
              openSuggestions.map((suggestion) => (
                <div
                  ...
                  role="button"
                  tabIndex={0}
                  className={`w-full cursor-pointer rounded-lg border bg-white dark:bg-slate-800 p-4 text-left transition-colors ${
                ...
                >
                  <div className="mb-2 flex items-start justify-between gap-3">
                    <span className="text-xs font-mono text-gray-500 dark:text-slate-400">{suggestion.status}</span>
                    ...
                    <div className="mt-3 flex gap-2">
                      {onSuggestionAccept && (
                        <button
                          ...
                          className="rounded bg-green-100 dark:bg-green-900/40 px-2 py-1 text-xs text-green-700 dark:text-green-300 hover:bg-green-200 dark:hover:bg-green-900/60"
                        >
                          Accept
                        </button>
                      )}
                      {onSuggestionReject && (
                        <button
                          ...
                          className="rounded bg-red-100 dark:bg-red-900/40 px-2 py-1 text-xs text-red-700 dark:text-red-300 hover:bg-red-200 dark:hover:bg-red-900/60"
                        >
                          Reject
                        </button>
                      )}
                      {onSuggestionArchive && (
                        <button
                          ...
                          className="rounded bg-slate-100 dark:bg-slate-700 px-2 py-1 text-xs text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600"
                        >
                          Archive
                        </button>
                      )}
