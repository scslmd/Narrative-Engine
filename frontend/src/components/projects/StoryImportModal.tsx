...
export function StoryImportModal({ isOpen, onClose }: StoryImportModalProps): React.ReactElement {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let response: StoryImportResponse | { project_id: string; status: 'completed' | 'failed'; error: string | null };
      ...
      if (response.status === 'completed') {
        ...
        setError(('error' in response ? response.error : 'message' in response ? (response as StoryImportResponse).message : null) || 'Import failed.');
      }
    } catch (err: unknown) {
            ...
            >
              {isImporting ? (
                ...
                <>
                  ...
                  {importMode === 'mythos' ? 'Extract Mythos' : 'Import Story'}
                </>
              )}
            </button>
          </div>
