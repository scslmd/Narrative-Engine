...
import type { ScenePlan } from '../../types/planning';
import type { ApiError } from '../../lib/api';
import { ErrorBanner } from '../ui/ErrorBanner';
import { LoadingState } from '../ui/LoadingState';
import { EmptyState } from '../ui/EmptyState';

export interface ScenePlanSectionProps {
  plans: ScenePlan[];
  isLoading: boolean;
  error: ApiError | null;
  onRetry: () => void;
...
export function ScenePlanSection({
  plans,
  isLoading,
  createOpen,
  createTitle,
        ...
        <div className="flex justify-end mb-2">
          {!createOpen ? (
            ...
            <div className="flex flex-col gap-2 w-80">
              <input type="text" placeholder="Scene title" value={createTitle} onChange={(e) => onCreateTitleChange(e.target.value)} className={inputClass} />
              ...
              <input type="text" placeholder="Chapter ID (optional)" value={createChapterId} onChange={(e) => onCreateChapterIdChange(e.target.value)} className={inputClass} />
    ...
    >
      {error ? (
        ...
        <LoadingState isLoading={isLoading}>
          {plans.length === 0 ? (
            <EmptyState title="No scenes yet" description="Define granular story beats with scenes." actionLabel="Add Scene" onAction={onCreateOpen} />
          ) : (
            <div className="space-y-2">
              {plans.map((scene, index) => (
                <div key={scene.scene_id} className={cardClass}>
                  {editOpenId === scene.scene_id ? (
        ...
        {scene.chapter_id && <span className="text-xs text-subtle">Chapter: {scene.chapter_id}</span>}
                      ...
                      <div className="flex items-center gap-1 flex-shrink-0">
                        ...
                        <button onClick={() => onEditOpen(scene)} className={`text-xs px-2 py-0.5 rounded ${isDark ? 'text-slate-400 hover:text-slate-200 hover:bg-slate-800' : 'text-slate-400 hover:text-slate-700 hover:bg-slate-100'}`}>Edit</button>
                      </div>
                    </div>
                  )}
                </div>
