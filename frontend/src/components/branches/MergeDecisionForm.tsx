...
export function MergeDecisionForm({ sourceBranch, targetBranch, onSubmit, onCancel }: MergeDecisionFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Source Branch</label>
        <input
          type="text"
          value={sourceBranch.branch_name}
          disabled
          className="w-full px-3 py-2 border rounded-md bg-gray-50 dark:bg-slate-900 text-gray-600 dark:text-slate-400"
        />
      </div>

      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Target Branch</label>
        <input
          type="text"
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Merge Rationale</label>
        <textarea
      ...
      <div className="flex gap-2">
        ...
        <button
          type="button"
          onClick={onCancel}
          className="px-4 py-2 bg-gray-600 dark:bg-slate-500 text-white rounded-md hover:bg-gray-700 dark:hover:bg-slate-600"
        >
          Cancel
        </button>
      </div>
