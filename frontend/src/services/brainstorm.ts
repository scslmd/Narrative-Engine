...
export async function getBrainstormItems(projectId: string): Promise<BrainstormItem[]> {
  const response = await api.get('/v1/story-development/brainstorm/items', {
...
export async function createBrainstormItem(
  ...
  const response = await api.post('/v1/story-development/brainstorm/items', request);
...
export async function clusterBrainstormItems(
  ...
  const response = await api.post('/v1/story-development/brainstorm/items/cluster', request);
...
export async function promoteBrainstormItem(
  ...
  const response = await api.post('/v1/story-development/brainstorm/items/promote', {
    item_id: itemId,
